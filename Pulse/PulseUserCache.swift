//
//  PulseUserCache.swift
//  Pulse
//
//  Created by Sidharth Tiwari on 7/10/17.
...
open class ImageCache {
    ...
    open func retrieveImage(forKey key: String,
                            completion: ((UIImage?, CacheType) -> ())?) -> RetrieveImageDiskTask?
    {
        ...
        if let image = self.retrieveImageInMemoryCache(forKey: key) {
            ...
            block = DispatchWorkItem(block: {
                ...
                if let image = sSelf.retrieveImageInDiskCache(forKey: key) {
    ...
    /**
     ...
     - returns: The image object if it is cached, or `nil` if there is no such key in the cache.
     */
    open func retrieveImageInDiskCache(forKey key: String) -> UIImage? {
        return diskImage(forComputedKey: key)
    }
...
extension ImageCache {
    ...
    func diskImage(forComputedKey key: String) -> UIImage? {
